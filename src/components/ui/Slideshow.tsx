...
export const HoverSliderContext = React.createContext<HoverSliderContextValue | undefined>(undefined);
...
>(({ text, index, children, className, ...props }, ref) => {
  ...
  return (
    <span
      className={cn('relative inline-flex items-center origin-bottom overflow-hidden py-1', className)}
      {...props}
      ref={ref}
      onMouseEnter={handleMouse}
    >
      {characters.map((char, index) => (
        <span key={`${char}-${index}`} className="relative inline-block overflow-hidden py-0.5">
          <MotionConfig
            transition={{
              delay: index * 0.025,
              duration: 0.3,
